...
const { subtitles, proxyStream } = require('./addic7ed');
...
//  /show/6849/original/134973/11.srt
app.get('/:encodedreferer/:encodedlink.srt?', limiter, (req, res) => {
	console.log(req.params);
	let { encodedreferer, encodedlink } = req.params;
	try {
		...
		proxyStream(link,referer).then(response => {
			res.send(response);
